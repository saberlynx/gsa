...
export const useRunQuickTask = () => {
  ...
  const runQuickTask = ({
    ...
    if (hasValue(alert_email) && alert_email.length > 0) {
      const createAlertInput = {
        name: `Email Alert for ${task_name} - ${rightNow}`,
        comment: 'Automatically generated by wizard',
        event: 'TASK_RUN_STATUS_CHANGED',
        eventData: {
          status: 'DONE',
    ...
    return alertPromise.then(alertId => {
      ...
      if (auto_start === '1') {
        ...
        const createScheduleInput = {
          name: `Schedule for ${task_name} - ${rightNow}`,
          icalendar: event.toIcalString(),
      ...
      return schedulePromise.then(resp => {
        ...
        const createTargetInput = {
          name: `Target for ${task_name} - ${rightNow}`,
          hosts: target_hosts,
          portListId: ALL_IANA_ASSIGNED_TCP,
          comment: 'Automatically generated by wizard',
          esxiCredentialId: esxi_credential.length > 0 ? esxi_credential : null,
          smbCredentialId: smb_credential.length > 0 ? smb_credential : null,
          sshCredentialId: ssh_credential.length > 0 ? ssh_credential : null,
          sshCredentialPort: ssh_port,
        };
